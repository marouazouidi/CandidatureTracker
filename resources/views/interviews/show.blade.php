    ...
    <div class="card">
        <div class="card-body">
            <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <dt class="text-xs font-medium text-dark-400 uppercase tracking-wider">{{ __('Type') }}</dt>
                    <dd class="mt-1 text-sm font-medium text-dark-600">{{ $interview->type }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium text-dark-400 uppercase tracking-wider">{{ __('Date') }}</dt>
                    <dd class="mt-1 text-sm text-dark-600">{{ $interview->interview_date->format('d/m/Y') }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium text-dark-400 uppercase tracking-wider">{{ __('Heure') }}</dt>
                    <dd class="mt-1 text-sm text-dark-600">{{ $interview->interview_time ? \Carbon\Carbon::parse($interview->interview_time)->format('H:i') : '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs font-medium text-dark-400 uppercase tracking-wider">{{ __('Résultat') }}</dt>
                    <dd class="mt-1">
            ...
            @if($interview->preparation_notes)
                <div class="mt-5 pt-5 border-t border-dark-100">
                    <dt class="text-xs font-medium text-dark-400 uppercase tracking-wider mb-2">{{ __('Notes de préparation') }}</dt>
                    <dd class="text-sm text-dark-600 whitespace-pre-wrap">{{ $interview->preparation_notes }}</dd>
                </div>
            @endif
        </div>
    </div>
